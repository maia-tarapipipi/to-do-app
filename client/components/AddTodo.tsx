    ...
    <>
      <form onSubmit={handleSubmit}>
        <input
          ...
          name="todo"
        />
        <div style={{ display: 'flex', margin: '0 20px', gap: '10px', alignItems: 'center', justifyContent: 'center' }}>
          <label htmlFor="priority">
            <h3>Priority:</h3>
          </label>
          <input onChange={handleChange} type="range" min="1" max="5" id="priority" name="priority" value={form.priority} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
          <button type="submit" style={{ alignContent: 'center' }}>Add</button>
        </div>
      </form>
    </>
  );
